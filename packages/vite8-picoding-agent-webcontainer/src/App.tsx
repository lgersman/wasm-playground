...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Watch the WebContainer filesystem for agent-driven changes
      watcher = wc.fs.watch(
        "/",
        { recursive: true },
        (_event, filename) => {
          const name = typeof filename === "string" ? filename : null;
          if (!stopped && name && !name.includes("node_modules")) {
  ...
  return (
    <div style={{ position: "relative", height: "100vh", width: "100vw" }}>
      <ResizablePanelGroup direction="horizontal" style={{ height: "100%", width: "100%" }}>
        {/* Left panel: Agent terminal running pi */}
        <ResizablePanel defaultSize={30} minSize={20}>
          <AgentTerminal webcontainer={webcontainer} />
        </ResizablePanel>
        <ResizableHandle style={{ width: "4px" }} />
        {/* Right panel: Editor + Preview + Terminal (same layout as vite8-webcontainer-demo) */}
