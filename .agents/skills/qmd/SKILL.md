...
metadata:
  author: tobi
  version: "2.0.0"
allowed-tools: Bash(vp exec qmd *), mcp__qmd__*
---
...
!`vp exec qmd status 2>/dev/null || echo "Not available: run 'vp install' first"`
...
```bash
vp exec qmd query "question"              # Auto-expand + rerank
vp exec qmd query $'lex: X\nvec: Y'       # Structured
vp exec qmd query $'expand: question'     # Explicit expand
vp exec qmd query --json --explain "q"    # Show score traces (RRF + rerank blend)
vp exec qmd search "keywords"             # BM25 only (no LLM)
vp exec qmd get "#abc123"                 # By docid
vp exec qmd multi-get "journals/2026-*.md" -l 40  # Batch pull snippets by glob
vp exec qmd multi-get notes/foo.md,notes/bar.md   # Comma-separated list, preserves order
```
...
```bash
vp install                                          # installs @tobilu/qmd from package.json
vp exec qmd collection add ~/notes --name notes
vp exec qmd embed
```
